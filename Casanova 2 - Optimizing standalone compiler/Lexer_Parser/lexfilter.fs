...
module (*internal*) CasanovaCompiler.Compiler.Lexfilter
...
let stringOfPos (p:Position) = sprintf "(%d:%d)" p.Line p.Column
let outputPos os (p:Position) = Printf.fprintf os "(%d:%d)" p.Line p.Column
...
type Context = 
    ...
    | CtxtIf of Position
    | CtxtSelectOperator of Position
    | CtxtIntSelectOperator of Position
    | CtxtBodyOperator of Position
    | CtxtThen of Position  
    | CtxtElse of Position 


    | CtxtDo of Position
    | CtxtWait of Position
    | CtxtParallel of Position
    | CtxtYield of Position
    ...
    | CtxtParen of CasanovaCompiler.Parser.token * Position 
    ...
    member c.StartPos = 
        ...
        | CtxtLetDecl (_,p) | CtxtWait p | CtxtParallel p | CtxtYield p | CtxtDo p | CtxtInterfaceHead p | CtxtTypeDefns p | CtxtParen (_,p) | CtxtMemberHead p | CtxtMemberBody p
        | CtxtWithAsLet p
        | CtxtWithAsAugment p
        | CtxtMatchClauses (_,p) | CtxtIf p | CtxtMatch p | CtxtSelectOperator p | CtxtIntSelectOperator p | CtxtFor p | CtxtWhile p | CtxtWhen p | CtxtFunction p | CtxtFun p | CtxtTry p | CtxtThen p | CtxtBodyOperator p | CtxtElse p | CtxtVanilla (p,_)
    ...
    override c.ToString() = 
        ...
        | CtxtIf _ -> "if"
        | CtxtMatch _ -> "match"
        | CtxtSelectOperator _ -> ".|"
        | CtxtIntSelectOperator _ -> "!|"
        | CtxtFor _ -> "for"
...
let rec isIfBlockContinuator token =
    ...
    //    elif ...
    //    else ... 
    | THEN | ELSE | BODY_OPERATOR | ELIF -> true  
    // Likewise 
    //    if  ... then  (
    //    ) elif begin 
    //    end else ... 
...
let rec isWhereBlockContinuator token =
    match token with 
    // The following arise during reprocessing of the inserted tokens when we hit a DONE
    | ORIGHT_BLOCK_END | OBLOCKEND | ODECLEND -> true 
    | ODUMMY(token) -> isWhereBlockContinuator(token)
    | _ -> false
...
let rec isNamespaceContinuator token =
    ...
    //     namespace <-- here
    //     .... 
    | CasanovaCompiler.Parser.EOF _ | NAMESPACE -> false 
...
let rec isSeqBlockElementContinuator token =
    ...
    //              ),                              <------- NOTE RPAREN HERE
    //              Shortcut.CtrlO)
    | END | AND | WITH | THEN | BODY_OPERATOR | RPAREN | RBRACE | RBRACK | BAR_RBRACK | RQUOTE _ -> true 
...
// build a LexFilter
//--------------------------------------------------------------------------*)
type LexFilterImpl (lightSyntaxStatus:LightSyntaxStatus, compilingFsLib, lexer, lexbuf: Lexing.LexBuffer<char>)  = 
    ...
    let setLexbufState (p:LexbufState) =
        // if debug then printf "SET lex state to; %a\n" output_any p;  
    ...
    let startPosOfTokenTup (tokenTup:TokenTup) = 
        ...
        | CasanovaCompiler.Parser.EOF _ -> tokenTup.LexbufState.StartPos
    ...
    let peekInitial() =
        ...
        //if debug then printf "first token: initialLookaheadTokenLexbufState = %a\n" outputPos (startPosOfTokenTup initialLookaheadTokenTup)
    ...
    let detectJoinInCtxt stack =
        let rec check s = 
               match s with 
               | CtxtParen(CasanovaCompiler.Parser.LBRACE,_) :: _ -> true
               | (CtxtSeqBlock _ | CtxtDo _ | CtxtWait _ | CtxtParallel _ | CtxtYield _ | CtxtFor _) :: rest -> check rest
               | _ -> false
        match stack with 
    ...
    let pushCtxt tokenTup (newCtxt:Context) =
        let rec unindentationLimit strict stack = 
            ...
            | _,((CtxtWithAsAugment _ | CtxtThen _ | CtxtBodyOperator _ | CtxtElse _ | CtxtDo _ | CtxtWait _ | CtxtParallel _ | CtxtYield _)  :: rest)
                      -> unindentationLimit false rest
            ...
            | _,(CtxtParen ((BEGIN | LPAREN | LBRACK | LBRACE | LBRACK_BAR)      ,_) :: CtxtSeqBlock _ :: CtxtBodyOperator _ :: (CtxtSelectOperator _         as limitCtxt) ::  _)
            | _,(CtxtParen ((BEGIN | LPAREN | LBRACK | LBRACE | LBRACK_BAR)      ,_) :: CtxtSeqBlock _ :: CtxtBodyOperator _ :: (CtxtIntSelectOperator _         as limitCtxt) ::  _)
            ...
            | (CtxtIf   _ | CtxtElse _ | CtxtThen _), (CtxtIf _ as limitCtxt) :: _rest  
                      -> (limitCtxt.StartPos,limitCtxt.StartCol)
            | (CtxtSelectOperator   _ | CtxtBodyOperator _), (CtxtSelectOperator _ as limitCtxt) :: _rest  
                      -> (limitCtxt.StartPos,limitCtxt.StartCol)
            | (CtxtIntSelectOperator   _ | CtxtBodyOperator _), (CtxtIntSelectOperator _ as limitCtxt) :: _rest  
                      -> (limitCtxt.StartPos,limitCtxt.StartCol)
            ...
            | _,((CtxtInterfaceHead _ | CtxtNamespaceHead _ | CtxtModuleHead _ | CtxtException _ | CtxtModuleBody (_,false) | CtxtSelectOperator _ | CtxtIntSelectOperator _ | CtxtIf _ | CtxtWithAsLet _ | CtxtLetDecl _ | CtxtMemberHead _ | CtxtMemberBody _) as limitCtxt :: _) 
                      -> (limitCtxt.StartPos,limitCtxt.StartCol + 1) 
        ...
        //if debug then printf "--> pushing, stack = %A\n" newOffsideStack
    ...
    let popCtxt() = 
        ...
        |  [] -> ()
        | _ :: rest -> 
             //if debug then printf "<-- popping Context(%A), stack = %A\n" h rest
             offsideStack <- rest
        ...
        | INFIX_COMPARE_OP "</" | LESS _ -> 
            ...
            if isAdjacent tokenTup lookaheadTokenTup then 
                ...
                let rec scanAhead nParen = 
                    ...
                    | CasanovaCompiler.Parser.EOF _ | SEMICOLON_SEMICOLON -> false 
    ...
    let rec hwTokenFetch (useBlockRule) =
        ...
        let rec isLongIdentEquals (token : CasanovaCompiler.Parser.token) = 
            match token with 
            | CasanovaCompiler.Parser.GLOBAL
            | CasanovaCompiler.Parser.IDENT _ -> 
                let rec loop() = 
                    let tokenTup = popNextTokenTup()
                    let res = 
                        match tokenTup.Token with 
        ...
        let thereIsACtxtMemberBodyOnTheStackAndWeShouldPopStackForUpcomingMember ctxtStack = 
            ...
            if not(List.exists (function CtxtMemberBody _ -> true | _ -> false) ctxtStack) then
                ...
                elif count (function CtxtParen(LPAREN,_) -> true | _ -> false) ctxtStack >= 2 then
        ...
        let tokenForcesHeadContextClosure token stack = 
            nonNil stack &&
            match token with 
            | CasanovaCompiler.Parser.EOF _ -> true
        ...
        let insertComingSoonTokens(keywordName, comingSoon, isHere) =
            ...
            if not compilingFsLib then
                ...
                while not offsideStack.IsEmpty && (not(nextOuterMostInterestingContextIsNamespaceOrModule(offsideStack))) &&
                    ...
                    match offsideStack.Head with
                    | CtxtParen _  ->
                        if debug then printf "%s at %a terminates CtxtParen()\n" keywordName outputPos tokenStartPos
                        popCtxt()
                    | CtxtSeqBlock(_,_,AddBlockEnd) ->  
                        popCtxt()
                        effectsToDo <- (fun() -> 
                            if debug then printf "--> because %s is coming, inserting OBLOCKEND\n" keywordName
                            delayTokenNoProcessing (tokenTup.UseLocation(OBLOCKEND))) :: effectsToDo
                    | CtxtSeqBlock(_,_,NoAddBlockEnd) ->  
                        if debug then printf "--> because %s is coming, popping CtxtSeqBlock\n" keywordName
                        popCtxt()
                    | CtxtSeqBlock(_,_,AddOneSidedBlockEnd) ->  
                        popCtxt()
                        effectsToDo <- (fun() -> 
                            if debug then printf "--> because %s is coming, inserting ORIGHT_BLOCK_END\n" keywordName
                            delayTokenNoProcessing (tokenTup.UseLocation(ORIGHT_BLOCK_END))) :: effectsToDo
                    | CtxtVanilla _ ->  
                        if debug then printf "--> because %s is coming, popping CtxtVanilla\n" keywordName
                        popCtxt()
            ...
            if debug then printf "inserting 6 copies of %+A before %+A\n" comingSoon isHere
        ...
        |  DONE, (CtxtDo offsidePos :: _) -> 
            if debug then printf "DONE at %a terminates CtxtDo(offsidePos=%a)\n" outputPos tokenStartPos outputPos offsidePos
            popCtxt()
        ...
        |  END, (CtxtWithAsAugment(offsidePos) :: _) 
            ...
            if debug then printf "END at %a terminates CtxtWithAsAugment()\n" outputPos tokenStartPos
            popCtxt()
        ...
        | _, (CtxtNamespaceHead (namespaceTokenPos, prevToken) :: _) -> 
            ...
            | IDENT _, DOT when namespaceTokenPos.Column < tokenStartPos.Column -> 
                ...
                returnToken tokenLexbufState token
            | _ -> 
                if debug then printf "CtxtNamespaceHead: pushing CtxtSeqBlock\n"
                popCtxt()
                // Don't push a new context if next token is EOF, since that raises an offside warning
                match tokenTup.Token with 
                | CasanovaCompiler.Parser.EOF _ -> 
                    returnToken tokenLexbufState token
                | _ -> 
                    delayToken tokenTup
        ...
        | _, (CtxtModuleHead (moduleTokenPos,prevToken) :: _)  ->
            ...
            | IDENT _, DOT when moduleTokenPos.Column < tokenStartPos.Column -> 
                ...
                returnToken tokenLexbufState token
            | _, (EQUALS | COLON) -> 
                if debug then printf "CtxtModuleHead: COLON/EQUALS, pushing CtxtModuleBody and CtxtSeqBlock\n"
                popCtxt()
                pushCtxt tokenTup (CtxtModuleBody (moduleTokenPos,false))
                pushCtxtSeqBlock(true,AddBlockEnd)
                returnToken tokenLexbufState token
            | _ -> 
                if debug then printf "CtxtModuleHead: start of file, CtxtSeqBlock\n"
                popCtxt()
                // Don't push a new context if next token is EOF, since that raises an offside warning
                match tokenTup.Token with 
                | CasanovaCompiler.Parser.EOF _ -> 
        ...
        | _, (CtxtDo offsidePos :: _) 
            ...
            if debug then printf "token at column %d is offside from DO(offsidePos=%a)! delaying token, returning ODECLEND\n" tokenStartCol outputPos offsidePos
            popCtxt()
            insertToken ODECLEND

        | _, (CtxtWait offsidePos :: _) 
                when isSemiSemi || (if isDoContinuator token then tokenStartCol + 1 else tokenStartCol) <= offsidePos.Column -> 
            if debug then printf "token at column %d is offside from WAIT(offsidePos=%a)! delaying token, returning ODECLEND\n" tokenStartCol outputPos offsidePos
            popCtxt()
            insertToken ODECLEND

        | _, (CtxtParallel offsidePos :: _) 
                when isSemiSemi || (if isDoContinuator token then tokenStartCol + 1 else tokenStartCol) <= offsidePos.Column -> 
            if debug then printf "token at column %d is offside from PARALLEL_OPERATOR(offsidePos=%a)! delaying token, returning ODECLEND\n" tokenStartCol outputPos offsidePos
            popCtxt()
            insertToken ODECLEND
        
        | _, (CtxtYield offsidePos :: _) 
                when isSemiSemi || (if isDoContinuator token then tokenStartCol + 1 else tokenStartCol) <= offsidePos.Column -> 
            if debug then printf "token at column %d is offside from YIELD(offsidePos=%a)! delaying token, returning ODECLEND\n" tokenStartCol outputPos offsidePos
            popCtxt()
            insertToken ODECLEND
        ...
        | _, (CtxtInterfaceHead offsidePos :: _) 
            ...
            if debug then printf "token at column %d is offside from INTERFACE(offsidePos=%a)! pop and reprocess\n" tokenStartCol outputPos offsidePos
            popCtxt()
            reprocess()
        ...
        | _, ((CtxtNamespaceBody offsidePos) :: _) when (* isSemiSemi || *) (if isNamespaceContinuator token then tokenStartCol + 1 else tokenStartCol) <= offsidePos.Column -> 
            if debug then printf "token at column %d is offside from NAMESPACE with offsidePos %a! delaying token\n" tokenStartCol outputPos offsidePos
            popCtxt()
            reprocess()
        ...
        | _, ((CtxtMemberBody(offsidePos)) :: _) when isSemiSemi || tokenStartCol <= offsidePos.Column -> 
            if debug then printf "token at column %d is offside from MEMBER/OVERRIDE head with offsidePos %a!\n" tokenStartCol outputPos offsidePos
            popCtxt()
            insertToken ODECLEND
        ...
        | _, (CtxtSelectOperator offsidePos :: _) 
                    when isSemiSemi || (if isIfBlockContinuator token then  tokenStartCol + 1 else tokenStartCol) <= offsidePos.Column -> 
            if debug then printf "offside from CtxtSelectOperator \n"
            popCtxt()
            reprocess()

        | _, (CtxtIntSelectOperator offsidePos :: _) 
                    when isSemiSemi || (if isIfBlockContinuator token then  tokenStartCol + 1 else tokenStartCol) <= offsidePos.Column -> 
            if debug then printf "offside from CtxtIntSelectOperator \n"
            popCtxt()
            reprocess()
        ...
        | _, (CtxtWithAsAugment(offsidePos) :: _) 
            ...
            if debug then printf "offside from CtxtWithAsAugment, isWithAugmentBlockContinuator = %b\n" (isWithAugmentBlockContinuator token)
            popCtxt()
            insertToken ODECLEND 
        ...
        | _, (CtxtFor offsidePos :: _) 
            ...
            if debug then printf "offside from CtxtFor\n"
            popCtxt()
            reprocess()
        ...
        | _, (CtxtWhile offsidePos :: _) 
            ...
            if debug then printf "offside from CtxtWhile\n"
            popCtxt()
            reprocess()
        ...
        | _, (CtxtBodyOperator offsidePos :: _) when isSemiSemi ||  (if isThenBlockContinuator token then  tokenStartCol + 1 else tokenStartCol)<= offsidePos.Column -> 
            if debug then printf "offside from CtxtBodyOperator, popping\n"
            popCtxt()
            reprocess()

        | _, (CtxtWhen offsidePos :: _) when isSemiSemi ||  (if isWhereBlockContinuator token then  tokenStartCol + 1 else tokenStartCol)<= offsidePos.Column -> 
            if debug then printf "offside from CtxtWhere, popping\n"
            popCtxt()
            reprocess()
        ...
        |  NAMESPACE,(_ :: _) -> 
            if debug then printf "NAMESPACE: entering CtxtNamespaceHead, awaiting end of long identifier to push CtxtSeqBlock\n" 
        ...
        |  EXCEPTION,(_ :: _) -> 
            if debug then printf "EXCEPTION: entering CtxtException(%a)\n" outputPos tokenStartPos
        ...
        |  (CREATE | RULE | VAL | STATIC | ABSTRACT | MEMBER | OVERRIDE | DEFAULT), ctxtStack when thereIsACtxtMemberBodyOnTheStackAndWeShouldPopStackForUpcomingMember ctxtStack -> 
            if debug then printf "STATIC/MEMBER/OVERRIDE/DEFAULT: already inside CtxtMemberBody, popping all that context before starting next member...\n"
            ...
            if debug then printf "...STATIC/MEMBER/OVERRIDE/DEFAULT: finished popping all that context\n"
            hwTokenFetch(useBlockRule)
        ...
        //  val ... ~~~> CtxtMemberHead 
        |  (CREATE | RULE | VAL | STATIC | ABSTRACT | MEMBER | OVERRIDE | DEFAULT),(ctxt :: _) when (match ctxt with CtxtMemberHead _ -> false | _ -> true) -> 
            if debug then printf "STATIC/MEMBER/OVERRIDE/DEFAULT: entering CtxtMemberHead, awaiting EQUALS to go to CtxtSeqBlock (%a)\n" outputPos tokenStartPos
        ...
        |  (PUBLIC | PRIVATE | INTERNAL),(_ctxt :: _) when (match peekNextToken() with NEW -> true | _ -> false) -> 
            if debug then printf "PUBLIC/PRIVATE/INTERNAL NEW: entering CtxtMemberHead, awaiting EQUALS to go to CtxtSeqBlock (%a)\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtMemberHead(tokenStartPos))
            returnToken tokenLexbufState token
        ...
        | NEW, ctxt :: _  when (match peekNextToken() with LPAREN -> true | _ -> false) &&  (match ctxt with CtxtMemberHead _ -> false | _ -> true)   -> 
            if debug then printf "NEW: entering CtxtMemberHead, awaiting EQUALS to go to CtxtSeqBlock (%a)\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtMemberHead(tokenStartPos))
            returnToken tokenLexbufState token
        ...
        | (LAZY | ASSERT), _ ->  
            if isControlFlowOrNotSameLine() then 
                if debug then printf "LAZY/ASSERT, pushing CtxtSeqBlock\n"
                pushCtxtSeqBlock(true,AddBlockEnd)
        ...
        | EQUALS, ((CtxtVanilla (_,true)) :: (CtxtSeqBlock _) :: (CtxtWithAsLet _ | CtxtParen(LBRACE,_)) :: _) ->  
            if debug then printf "CtxtLetDecl/CtxtWithAsLet: EQUALS, pushing CtxtSeqBlock\n"
        ...
        | EQUALS, ((CtxtMemberHead(offsidePos)) :: _) ->  
            if debug then printf "CtxtMemberHead: EQUALS, pushing CtxtSeqBlock\n"
        ...
        | RARROW, ctxts 
                ...
                when (match ctxts with 
                        ...
                        // comprehension 
                        | (CtxtSeqBlock _ :: (CtxtDo _ |CtxtWait _ | CtxtParallel _ | CtxtYield _ | CtxtWhile _ | CtxtFor _ | CtxtWhen _ | CtxtMatchClauses _  | CtxtTry _ | CtxtThen _ | CtxtBodyOperator _ | CtxtElse _) :: _) -> true 
                        | _ -> false) ->
        ...
        | (DO | DO_BANG), _ -> 
            if debug then printf "DO: pushing CtxtSeqBlock, tokenStartPos = %a\n" outputPos tokenStartPos
        ...
        | (WAIT | WHEN), _ -> 
            if debug then printf "WAIT: pushing CtxtSeqBlock, tokenStartPos = %a\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtWait (tokenStartPos))
            pushCtxtSeqBlock(true,AddBlockEnd)
            returnToken tokenLexbufState (match token with WAIT -> OWAIT | WHEN -> OWHEN | _ -> failwith "unreachable")

        | PARALLEL_OPERATOR, _ ->
            if debug then printf "PARALLEL_OPERATOR: pushing CtxtSeqBlock, tokenStartPos = %a\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtParallel (tokenStartPos))
            pushCtxtSeqBlock(true,AddBlockEnd)
            returnToken tokenLexbufState (match token with PARALLEL_OPERATOR -> OPARALLEL_OPERATOR | _ -> failwith "unreachable")

        | YIELD, _ -> 
            if debug then printf "YIELD: pushing CtxtSeqBlock, tokenStartPos = %a\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtYield (tokenStartPos))
            pushCtxtSeqBlock(true,AddBlockEnd)
            returnToken tokenLexbufState (match token with YIELD -> OYIELD | _ -> failwith "unreachable")
        ...
        | FINALLY, (CtxtTry _ :: _)  -> 
            if debug then printf "FINALLY, pushing pushCtxtSeqBlock, tokenStartPos = %a\n" outputPos tokenStartPos
            pushCtxtSeqBlock(true,AddBlockEnd)
            returnToken tokenLexbufState token
        ...
        | BODY_OPERATOR,_  -> 
            if debug then printf "BODY_OPERATOR, replacing BODY_OPERATOR with OBODY_OPERATOR, pushing CtxtSeqBlock;CtxtBodyOperator(%a)\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtBodyOperator(tokenStartPos))
            pushCtxtSeqBlock(true,AddBlockEnd)
            returnToken tokenLexbufState OBODY_OPERATOR
        ...
        | ELSE, _   -> 
            ...
            | IF when isSameLine() ->
                ...
                if debug then printf "ELSE IF: replacing ELSE IF with ELIF, pushing CtxtIf, CtxtVanilla(%a)\n" outputPos tokenStartPos
        ...
        | SELECT_OPERATOR, _   -> 
            if debug then printf "SELECT_OPERATOR, pushing CtxtSelectOperator(%a)\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtSelectOperator (tokenStartPos))
            returnToken tokenLexbufState token

        | INT_SELECT_OPERATOR, _   -> 
            if debug then printf "INT_SELECT_OPERATOR, pushing CtxtIntSelectOperator(%a)\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtIntSelectOperator (tokenStartPos))
            returnToken tokenLexbufState token
        ...
        | FOR, _   -> 
            if debug then printf "FOR, pushing CtxtFor(%a)\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtFor (tokenStartPos))
            returnToken tokenLexbufState token

        | WHILE, _   -> 
            if debug then printf "WHILE, pushing CtxtWhile(%a)\n" outputPos tokenStartPos
        ...
        | INTERFACE, _  -> 
            ...
            | DEFAULT | OVERRIDE | INTERFACE | NEW | TYPE | STATIC | END | MEMBER | ABSTRACT  | INHERIT | LBRACK_LESS -> 
                if debug then printf "INTERFACE, pushing CtxtParen, tokenStartPos = %a, lookaheadTokenStartPos = %a\n" outputPos tokenStartPos outputPos lookaheadTokenStartPos
                pushCtxt tokenTup (CtxtParen (token,tokenStartPos))
                pushCtxtSeqBlock(true,AddBlockEnd)
                returnToken tokenLexbufState token
            ...
            | _ -> 
                if debug then printf "INTERFACE, pushing CtxtInterfaceHead, tokenStartPos = %a, lookaheadTokenStartPos = %a\n" outputPos tokenStartPos outputPos lookaheadTokenStartPos
                pushCtxt tokenTup (CtxtInterfaceHead(tokenStartPos))
                returnToken tokenLexbufState OINTERFACE_MEMBER

        | CLASS, _   -> 
            if debug then printf "CLASS, pushing CtxtParen(%a)\n" outputPos tokenStartPos
            pushCtxt tokenTup (CtxtParen (token,tokenStartPos))
            pushCtxtSeqBlock(true,AddBlockEnd)
            returnToken tokenLexbufState token

        | WORLD_ENTITY,_ -> 
            insertComingSoonTokens("WORLD_ENTITY", TYPE_COMING_SOON, WORLD_ENTITY)
            if debug then printf "WORLD_ENTITY, pushing CtxtTypeDefns(%a)\n" outputPos tokenStartPos
    ...
    and rulesForBothSoftWhiteAndHardWhite(tokenTup:TokenTup) = 
          ...
          |  IDENT _ when (nextTokenIsAdjacentLParenOrLBrack tokenTup).IsSome ->
              let dotTokenTup = peekNextTokenTup()
              if debug then printf "inserting HIGH_PRECEDENCE_PAREN_APP at dotTokenPos = %a\n" outputPos (startPosOfTokenTup dotTokenTup)
              let hpa = 
          ...
          |  (DELEGATE | IDENT _ | IEEE64 _ | IEEE32 _ | DECIMAL _ | INT8 _ | INT16 _ | INT32 _ | INT64 _ | NATIVEINT _ | UINT8 _ | UINT16 _ | UINT32 _ | UINT64 _ | BIGNUM _) when peekAdjacentTypars false tokenTup ->
              ...
              if debug then printf "softwhite inserting HIGH_PRECEDENCE_TYAPP at dotTokenPos = %a\n" outputPos (startPosOfTokenTup lessTokenTup)
...
type LexFilter (lightSyntaxStatus:LightSyntaxStatus, compilingFsLib, lexer : (Lexing.LexBuffer<char> -> CasanovaCompiler.Parser.token) , lexbuf: Lexing.LexBuffer<char>)  = 
