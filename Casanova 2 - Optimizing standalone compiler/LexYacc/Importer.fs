...
let casanova_tokenizer (filename : string) error_logger : Microsoft.FSharp.Text.Lexing.LexBuffer<char> -> (CasanovaCompiler.Compiler.Lexhelp.lexargs -> bool -> LexBuffer<char> -> CasanovaCompiler.Parser.token)->LexBuffer<char>->CasanovaCompiler.Parser.token = 
        (fun lexbuf (token : (CasanovaCompiler.Compiler.Lexhelp.lexargs -> bool -> LexBuffer<char> -> CasanovaCompiler.Parser.token)) ->
